//com_example_camera2mediacodecvideotry2
#include <jni.h>
#include <string>
...
Java_com_example_camera2mediacodecvideotry4_MainActivity_yuvToBuffer(JNIEnv *env, jobject instance,
    ...
    if (yPixelStride == 1) {
        ...
        // other data in between each pixel.
        for (int y = 0; y < imgHeight; y++)
    ...
    auto *I420Buff = (uint8_t *) malloc(sizeof(uint8_t) * imgWidth * imgHeight +
                                        2 * (imgWidth + 1) / 2 * (imgHeight + 1) / 2);
